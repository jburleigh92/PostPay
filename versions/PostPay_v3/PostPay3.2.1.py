...
EXCEL_PATH = '/Users/jasonburleigh/postpay/programdirectory/all_payments.xlsx'
...
def write_database_to_excel():
    ...
    print(f"The database has been written to {EXCEL_PATH}")
...
def move_email_to_processed(service, msg_id, user_id='me', label_name='Processed'):
    try:
        ...
        # Move the message to the processed label
        service.users().messages().modify(
            userId=user_id,
            id=msg_id,
            body={'addLabelIds': [label_id], 'removeLabelIds': ['INBOX']}
        ).execute()
        print(f"Email {msg_id} moved to '{label_name}' label.")
    except Exception as e:
        print(f"Error moving email {msg_id} to '{label_name}' label: {e}")

def archive_old_payments(service):
    ...
    for message in messages:
        ...
        if details is None:
            continue
        if is_payment_logged(msg_id):
            move_email_to_processed(service, msg_id)
    ...
    while True:
        ...
        for message in messages:
            ...
            if formatted_message not in posted_messages and not is_payment_logged(msg_id):
                ...
                move_email_to_processed(service, msg_id)  # Move email to processed folder
            else:
